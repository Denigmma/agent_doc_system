...
def build_client_from_env(model_name: str, base_url_env: str = "OPENROUTER_BASE_URL", key_env: str = "API_KEY_OPENROUTER") -> ModelClient:
    load_dotenv(dotenv_path=Path(__file__).resolve().parents[3] / ".env")
    api_key = os.getenv(key_env) or os.getenv("OPENROUTER_API_KEY")
    if not api_key:
        raise ValueError(f"Neither {key_env} nor OPENROUTER_API_KEY is set.")
